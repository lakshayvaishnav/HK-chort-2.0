...
function fillOrder(
  ...
  // if there aren't enough orders to be matched max fill quantity will be lower than the quantity...
  ...
  // Attempts to fill the order by matching it against existing sell (ask) orders in the order book, starting with the lowest-priced ones.
  if (side === "buy") {
...
function getFillAmount(
  ...
  // how much order can be immediately matched in the existing orderbook...
  let filled = 0;
  if (side === "buy") {
    orderbook.asks.forEach((o) => {
      if (o.price < price) {
